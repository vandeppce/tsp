...
def updateBestPath(bestPath, points, MAXCOUNT):
    count = 0
    best_distance = 0
    distances = []
    while count < MAXCOUNT:
        ...
        best_distance = calPathDist(bestPath, points)
        distances.append(best_distance)
    return bestPath, best_distance, distances
...
def opt2(points, MAXCOUNT):
    # 随便选择一条可行路径
    bestPath = np.arange(0, len(points))
    bestPath = np.append(bestPath, 0)
    bestPath, best_distance, distances = updateBestPath(bestPath, points, MAXCOUNT)
